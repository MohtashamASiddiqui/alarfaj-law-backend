...
const PersonCard = ({ person }) => {
  return (
    <Link to={`/ar/team/${person.url}`} className="person-card-link">
      <div key={person._id} className="person-card">
        ...
        <h2 className="person-title  arabic-weight">{person.name}</h2>
        <p className="person-position  arabic-weight">{person.position}</p>
      </div>
    </Link>
  );
};
...
const YourComponent = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://www.arfajlaw.site/api/peopleArabic/read"
        );
  ...
  return (
    <div className="people-container" id="people">
      <h1 className="people-title arabic-weight">فريق العمل </h1>
      {peopleData.length > 0 && (
